...
def calculate_keypoint_transformation(args, img1_desc, img2_desc, img1_patch_idx, num_patches):
    ...
    if args.SPH:
        mixing_ratio = .8
        sim_1_to_2_feat = torch.matmul(img1_desc[...,:-3], img2_desc[...,:-3].permute(0, 2, 1))[0]
        sim_1_to_2_sph = torch.matmul(img1_desc[...,-3:], img2_desc[...,-3:].permute(0, 2, 1))[0]
        sim_1_to_2 = mixing_ratio * sim_1_to_2_feat + (1 - mixing_ratio) * sim_1_to_2_sph
    else:
...
def extract_kap_vals(args, img1_desc, img2_desc, img1_patch_idx, src_vis, trg_kps_full, num_patches, thresholds, normalize=True):
    # Calculate similarity matrix
    if args.SPH:
        mixing_ratio = .8
        sim_1_to_2_feat = torch.matmul(img1_desc[...,:-3], img2_desc[...,:-3].permute(0, 2, 1))[0]
    ...
    # Extract visible descriptors
    src_vis = src_vis.bool()
    sim_1_to_2_idxed = sim_1_to_2[img1_patch_idx]
    sim_1_to_2_idxed = sim_1_to_2_idxed[src_vis]
    # input(vis1.nonzero())

    # Adapt thresholds to feature map size
    desc_stride = num_patches / args.ANNO_SIZE
    kap_thresh = thresholds * desc_stride

    trg_kps = trg_kps_full[src_vis][:, :2] * desc_stride
    trg_vis = trg_kps_full[src_vis][:, 2].bool()

    k = trg_vis.size(0)
    t = kap_thresh.size(0)

    if normalize:
        sim_1_to_2_idxed = (sim_1_to_2_idxed - sim_1_to_2_idxed.min()) / (sim_1_to_2_idxed.max() - sim_1_to_2_idxed.min())
    similarity_maps = sim_1_to_2_idxed.view(-1, num_patches, num_patches)

    # Create a grid of coordinates
    y_coords, x_coords = torch.meshgrid(torch.arange(num_patches), torch.arange(num_patches), indexing='ij')
    y_coords = y_coords.unsqueeze(0).to(trg_kps.device)  # Shape (1, num_patches, num_patches)
    x_coords = x_coords.unsqueeze(0).to(trg_kps.device)  # Shape (1, num_patches, num_patches)

    # Expand kps and thresholds for broadcasting
    u = trg_kps[:, 0].view(k, 1, 1)  # Shape (k, 1, 1)
    v = trg_kps[:, 1].view(k, 1, 1)  # Shape (k, 1, 1)
    tau_squared = kap_thresh.view(t, 1, 1, 1) ** 2  # Shape (t, 1, 1, 1)

    # Calculate squared distances
    squared_distances = (x_coords - u) ** 2 + (y_coords - v) ** 2  # Shape (k, num_patches, num_patches)

    # Expand similarity maps for broadcasting
    similarity_maps_expanded = similarity_maps.unsqueeze(0)  # Shape (1, k, num_patches, num_patches)

    # Create masks for within and beyond tau
    within_tau_mask = squared_distances.unsqueeze(0) <= tau_squared  # Shape (t, k, num_patches, num_patches)
    beyond_tau_mask = ~within_tau_mask  # Shape (t, k, num_patches, num_patches)

    # Use masked operations to find max values
    max_within_tau = (similarity_maps_expanded * within_tau_mask).amax(dim=(-2, -1))  # Shape (t, k)
    max_beyond_tau = (similarity_maps_expanded * beyond_tau_mask).amax(dim=(-2, -1))  # Shape (t, k)

    # If the trg keypoint is not visible, we take the max over the whole image
    negative_preds = torch.where(trg_vis.unsqueeze(0), max_beyond_tau, torch.max(max_beyond_tau, max_within_tau))

    # only keep preds where the target point is visible
    positive_preds = max_within_tau[:, trg_vis]

    return max_within_tau, max_beyond_tau
