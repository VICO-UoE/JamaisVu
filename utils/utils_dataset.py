...
def load_eval_data(args, path, category, split, unseen_kps=False):
    if args.EVAL_DATASET == 'ap10k':
        ...
        files, kps, thresholds, used_kps = load_spair_data(path, args.ANNO_SIZE, category, split, args.TEST_SAMPLE, unseen_kps=unseen_kps)
...
# SPair-71K
def load_spair_data(path="data/SPair-71k", size=256, category='cat', split='test', subsample=None, unseen_kps=False):
    pair_ann_subpath = 'ExtraPairAnnotation' if unseen_kps else 'PairAnnotation'
    im_ann_subpath = 'ExtraImageAnnotation' if unseen_kps else 'ImageAnnotation'
    np.random.seed(42)
    pairs = sorted(glob(f'{path}/{pair_ann_subpath}/{split}/*:{category}.json'))
    if subsample is not None and subsample > 0:
        pairs = [pairs[ix] for ix in np.random.choice(len(pairs), subsample)]
    files = []
    thresholds = []
    kps = []
    category_anno = list(glob(f'{path}/{im_ann_subpath}/{category}/*.json'))[0]
    ...
    for pair in pairs:
        ...
        source_json_name = source_fn.replace('JPEGImages',im_ann_subpath).replace('jpg','json')
        target_json_name = target_fn.replace('JPEGImages',im_ann_subpath).replace('jpg','json')
